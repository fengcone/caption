...
public class Words {
	...
	public static void main(String[] args) throws Exception {
		...
		words.reverse(new File("D:\\Caption\\ass"));//需要遍历的文件夹
		words.deleteMap();
		words.handleMap();
		words.writeToFile(new File("D:/result.txt"));//写出到最后的结果
	}
	...
	private void handleMap(){
		...
		for (String string : set) {
			...
			Set<String> set3 =new HashSet<String>();//新建Set集合 用来存放在map里出现过的与string相关的单词
			set3.add(string);
			...
			if (b) {
				map.put(key.substring(0,key.length()-1), value); //放入新的key与新的value 到map
			}
		}
	}
	...
	private void handleTextfile(File file2) throws Exception {//从文件中拿到所有的单词并统计数量
		...
		while((string=br.readLine())!=null){
			...
			for (String string2 : strings) {
				...
				if (string2.length()>3&&string2.length()<14) {//对单词的长度进行筛选
					if(map.containsKey(string2)){
